...
contains
        ...
        SUBROUTINE newuoa_min(CALFUN,CALGREY,NF, X, RHOBEG, RHOEND, IPRINT, MAXFUN,PROBLEM,FLAG_Epsilonk)
                implicit none
                interface
                        subroutine CALFUN(i_x, o_f,i_problem)
                                real(kind=8), dimension(:) :: i_x
                                real(kind=8) :: o_f
                                character(len=15) :: i_problem
                        end subroutine

                        subroutine CALGREY(i_x,i_k,o_g,i_flag,i_eps,i_eps1,i_eps2,i_rand,i_problem,i_flag_epsilonk)
                                real(kind=8), dimension(:) :: i_x
                                real(kind=8) :: o_g
                                integer(kind=4) :: i_k
                                integer(kind=4) :: i_flag
                                double precision,dimension(:) :: i_eps
                                double precision,dimension(:) :: i_eps1
                                double precision,dimension(:) :: i_eps2
                                double precision,dimension(:) :: i_rand
                                character(len=15) :: i_problem
                                integer(kind=4) :: i_flag_epsilonk
                        end subroutine
                end interface
                real(kind=8), intent(inout), dimension(:) :: X
                real(kind=8), intent(in) :: RHOBEG, RHOEND
                integer, intent(in) :: IPRINT, MAXFUN, FLAG_Epsilonk
                ...
                call NEWUOA( CALFUN,CALGREY,NF,N,NPT,X,RHOBEG,RHOEND, &
                        IPRINT, MAXFUN, W,PROBLEM,FLAG_Epsilonk )
                deallocate( W )
        ...
        SUBROUTINE NEWUOA(CALFUN,CALGREY,NF,N,NPT,X,RHOBEG,RHOEND,IPRINT,MAXFUN,W,PROBLEM,FLAG_Epsilonk)
                implicit none
                integer, intent(in) :: N, NPT, IPRINT, MAXFUN,FLAG_Epsilonk
                ...
                CALL NEWUOB(CALFUN,CALGREY,NF, N,NPT,X,RHOBEG,RHOEND,IPRINT,MAXFUN,W(IXB),  &
                        W(IXO),W(IXN),W(IXP),W(IFV),W(IGQ),W(IHQ),W(IPQ),W(IBMAT), &
                        W(IZMAT),NDIM,W(ID),W(IVL),W(IW),PROBLEM,FLAG_Epsilonk)
                20 RETURN
        END SUBROUTINE NEWUOA



        SUBROUTINE NEWUOB(CALFUN,CALGREY,NF,N,NPT,X,RHOBEG,RHOEND,IPRINT,MAXFUN,XBASE, &
                        XOPT,XNEW,XPT,FVAL,GQ,HQ,PQ,BMAT,ZMAT,NDIM,D,VLAG,W,PROBLEM,FLAG_Epsilonk)

                implicit none
                integer, intent(in) :: N, NPT, IPRINT,MAXFUN, NDIM,FLAG_Epsilonk
                real(kind=8), intent(inout), dimension(:) :: X
                real(kind=8), intent(in) :: RHOBEG, RHOEND
                ...
                real(kind=8) :: HALF, ONE, TENTH, ZERO, RHOSQ, RECIP,RECIQ, XIPT, &
                        ...
                        DSTEP, SUMZ, VQUAD, G, G1, G2

                double precision :: Epsilonk

                character(len=15) :: PROBLEM
                real(kind=8),dimension(NPT) :: TEMP1
                real(kind=8),dimension(N) :: XBEG,XTEMP
                logical :: EXIST
                real*8,allocatable :: EPS(:),EPS1(:),EPS2(:),RAND(:)!声明存放误差的随机数的变量名

                ALLOCATE(EPS(MAXFUN))!定义存放误差的变量数组的长度
                call random_seed
                call random_number(EPS)!为这个存放误差的变量数组赋随机数

                ALLOCATE(EPS1(MAXFUN))!定义存放误差的变量数组的长度
                call random_seed
                call random_number(EPS1)!为这个存放误差的变量数组赋随机数

                ALLOCATE(EPS2(MAXFUN))!定义存放误差的变量数组的长度
                call random_seed
                call random_number(EPS2)!为这个存放误差的变量数组赋随机数

                ALLOCATE(RAND(MAXFUN))!定义存放随机数的变量数组的长度
                call random_seed
                call random_number(RAND)!为这个存放误差的变量数组赋随机数
                ...
                IF (KNEW .GT. 0) THEN
                        ...
                        IF (DABS(TEMP) .LE. 0.8D0) THEN!(6.17)
                ...
                310 IF (NF .GT. NFTEST) THEN
                        ...
                        GOTO 530
                END IF
                CALL CALFUN(X,F,PROBLEM)!获取新的插值点处的函数值

                IF (NF .GE. NPT+1) THEN
                        CALL CALGREY(X,NF-1,G2,FLAG,EPS,EPS1,EPS2,RAND,PROBLEM,FLAG_Epsilonk)

                        F=F+G2!将新的插值点处的函数值向前一个点处的函数值补齐
                        INQUIRE(FILE='history_F.csv', EXIST=EXIST)
                        IF (EXIST) THEN
                            OPEN(12,FILE='history_F.csv', STATUS='old', POSITION='append', ACTION='write')
                        ELSE
                            OPEN(12,FILE='history_F.csv', STATUS='new', ACTION='write')
                        END IF
                        G=0.0D0
                        DO I=1,N
                            G=G+(X(I))**2
                        ...
                        IF (NF .GE. NPT+2) THEN
                                ...
                                CALL CALGREY(XBEG,NF-2,G1,FLAG,EPS,EPS1,EPS2,RAND,PROBLEM,FLAG_Epsilonk)
                                CALL CALGREY(XBEG,NF-1,G2,FLAG,EPS,EPS1,EPS2,RAND,PROBLEM,FLAG_Epsilonk)

                                FBEG=FBEG+(G2-G1)!将初始点处的函数值FBEG向新插值点前一个点处的函数值补齐                                
                                !EPS1中存放的是混合噪声中的加法部分，EPS2中存放的是混合噪声中的乘法部分
								!EPS中存放的是Laplace噪声
								!RAND中存放的是混合噪声中每次产生的随机数，用以判断是做加法还是做乘法
								!FLAG_Epsilonk表示混合噪声或者单纯的Laplace噪声3.
                                DO I=1,N
                                    XTEMP(I)=XOPT(I)+XBASE(I)!获取当前最优值点的真实坐标
                                END DO
                                CALL CALGREY(XTEMP,NF-2,G1,FLAG,EPS,EPS1,EPS2,RAND,PROBLEM,FLAG_Epsilonk)
                                CALL CALGREY(XTEMP,NF-1,G2,FLAG,EPS,EPS1,EPS2,RAND,PROBLEM,FLAG_Epsilonk)

                                FOPT=(G2-G1)+FOPT!将最优点处的函数值FOPT向新插值点的前一个点处的函数值补齐

                                DO J=1,NPT
                                DO I=1,N
                                    XTEMP(I)=XPT(J,I)+XBASE(I)!获取当前插值点集合的真实坐标
                                END DO

                                CALL CALGREY(XTEMP,NF-2,G1,FLAG,EPS,EPS1,EPS2,RAND,PROBLEM,FLAG_Epsilonk)
                                CALL CALGREY(XTEMP,NF-1,G2,FLAG,EPS,EPS1,EPS2,RAND,PROBLEM,FLAG_Epsilonk)

                                FVAL(J)=(G2-G1)+FVAL(J)!将插值点集合处的函数值FVAL向新插值点的前一个点处的函数值补齐
                                END DO  
                        END IF 
                ELSE
                       CALL CALGREY(X,NPT,G1,FLAG,EPS,EPS1,EPS2,RAND,PROBLEM,FLAG_Epsilonk)
                        F=F+G1
                        ...
                        WRITE(12,*)(F-(G1-G))
                        CLOSE(12)!将F+G写入csv文件

                END IF

                IF (IPRINT .EQ. 3) THEN
                    PRINT 330, NF,F,(X(I),I=1,N)
                    330      FORMAT (/4X,'Function number',I6,'    F =',1PD23.15, &
                             '    The corresponding X array is:'/(2X,1PD23.15))
                END IF
                ...
                DO I=1,N
                XTEMP(I)=XOPT(I)+XBASE(I)+D(I)!获取新插值点的真实坐标
                END DO

                IF (NF .GE. NPT+1) THEN 
                    CALL CALGREY(XTEMP,NF,G2,FLAG,EPS,EPS1,EPS2,RAND,PROBLEM,FLAG_Epsilonk)
                    CALL CALGREY(XTEMP,NF-1,G1,FLAG,EPS,EPS1,EPS2,RAND,PROBLEM,FLAG_Epsilonk)

                    DIFF=F-FOPT+(G2-G1)-VQUAD!将DIFF值向新插值点处的函数值对齐
                END IF

                DIFFC=DIFFB
                DIFFB=DIFFA
                DIFFA=DABS((-G2+G1)+DIFF)!将未向新插值点处的函数值对齐的DIFF值存储至DIFFA中
                ...
                IF (JJ .NE. KNEW) THEN
                    DO K=1,N
                        XTEMP(K)=XBASE(K)+XPT(JJ,K)!获取插值点集合的真实坐标
                    END DO
                    ...
                    IF (NF .GE. NPT+1) THEN
                        ...
                        TEMP1(JJ)=(G2-G1)*ZMAT(JJ,J)!向量r发生改变（即DIFF发生改变之后），PQ也要相应的调整
                    ELSE
                        TEMP1(JJ)=0.0D0
                    END IF
                END IF
					
                IF (J .LT. IDZ) TEMP1(JJ)=-TEMP1(J)
                DO 440 K=1,NPT
                440 PQ(K)=PQ(K)+TEMP1(JJ)*ZMAT(K,J)!改变了TEMP1，PQ也就相应改变了
                ...
                IF (J .NE. KNEW) THEN
                    DO K=1,N
                        XTEMP(K)=XBASE(K)+XPT(J,K)!获取插值点集合的真实坐标
                    END DO
                    IF (NF .GE. NPT+1) THEN
                       CALL CALGREY(XTEMP,NF,G2,FLAG,EPS,EPS1,EPS2,RAND,PROBLEM,FLAG_Epsilonk)
                       CALL CALGREY(XTEMP,NF-1,G1,FLAG,EPS,EPS1,EPS2,RAND,PROBLEM,FLAG_Epsilonk)

                        GQ(I)=GQ(I)+(G2-G1)*BMAT(J,I)!向量r发生改变（即DIFF发生改变之后），梯度g也要相应的调整
                    END IF
                END IF
                END DO
                GQSQ=GQSQ+GQ(I)**2
        ...
        SUBROUTINE BIGDEN (N,NPT,XOPT,XPT,BMAT,ZMAT,IDZ,NDIM,KOPT, &
                        KNEW,D,W,VLAG,BETA,S,WVEC,PROD)!求解算法子程序
                ...
                IF (DS*DS .GT. 0.99D0*DD*SS) THEN!D和S接近平行，设置另外的S，对应公式（6.21），Select ωk which is the least of the ratios
                    KSAV=KNEW
                    DTEST=DS*DS/SS
                    DO 50 K=1,NPT
                    IF (K .NE. KOPT) THEN
                ...
                XOPTS=ZERO
                DO 80 I=1,N
                S(I)=TEMP*(DD*S(I)-DS*D(I))!DD^2*(S,S)+DS^2*(D,D)-2*DD*DS*(S,D)=DD*(DD*SS-DS*DS)
                XOPTD=XOPTD+XOPT(I)*D(I)
                80 XOPTS=XOPTS+XOPT(I)*S(I)!3-peng

                !     Set the coefficients of the first two terms of BETA.

                TEMPA=HALFXOPTD*XOPTD
                ...
                DO 110 K=1,NPT!dimension(WVEC)=(NPT+N) x 5
                TEMPA=ZERO
                TEMPB=ZERO
                TEMPC=ZERO
                DO 100 I=1,N
                ...
                PAR(1)=ONE
                DO 250 I=1,IU!公式(6.22)
                ANGLE=DFLOAT(I)*TEMP
                PAR(2)=DCOS(ANGLE)
                PAR(3)=DSIN(ANGLE)
                DO 230 J=4,8,2!J=[4,6,8]
                PAR(J)=PAR(2)*PAR(J-2)-PAR(3)*PAR(J-1)
                230 PAR(J+1)=PAR(2)*PAR(J-1)+PAR(3)*PAR(J-2)
                SUMOLD=SUM
                SUM=ZERO
                DO 240 J=1,9
                240 SUM=SUM+DENEX(J)*PAR(J)!DENEX为公式(6.22)的系数
                ...
                END IF
                ANGLE=TEMP*(DFLOAT(ISAVE)+STEP)!找到ANGLE
                ...
                IF (DABS(DENMAX) .LE. 1.1D0*DABS(DENSAV)) GOTO 340!根据收敛速度判断是否终止
                DENSAV=DENMAX
                ...
                W(K)=ZERO
                DO 350 J=1,5
                350 W(K)=W(K)+WVEC(K,J)*PAR(J)!公式(4.10)
                VLAG(KOPT)=VLAG(KOPT)+ONE
                RETURN
        ...
        SUBROUTINE BIGLAG (N,NPT,XOPT,XPT,BMAT,ZMAT,IDZ,NDIM,KNEW, &
                        DELTA,D,ALPHA,HCOL,GC,GD,S,W)!求解算法子程序
                IMPLICIT REAL(8) (A-H,O-Z)
                DIMENSION XOPT(*),XPT(NPT,*),BMAT(NDIM,*),ZMAT(NPT,*),D(*), &
                        HCOL(*),GC(*),GD(*),S(*),W(*)
                ...
                TAU=SCALE*(DABS(SP)+HALF*SCALE*DABS(DHD))!GC为梯度，GD为H*D
                ...
                SP=SP+D(I)*S(I)!(D,S)
                90 SS=SS+S(I)**2!S的模长
                TEMP=DD*SS-SP*SP!D的模长*S的模长-(D,S)^2     等价于向量夹角余弦
                ...
                SUM=HCOL(K)*SUM
                DO 120 I=1,N
                120 W(I)=W(I)+SUM*XPT(K,I)!W=G*S   即W=H*S
                CF1=ZERO
                CF2=ZERO
                CF3=ZERO
                CF4=ZERO
                ...
                ELSE IF (I .EQ. ISAVE+1) THEN
                    TEMPB=TAU
                END IF
                140 TAUOLD=TAU!从49个里面找出最大的TAU对应的angle
                ...
                DO 150 I=1,N
                D(I)=CTH*D(I)+STH*S(I)
                GD(I)=CTH*GD(I)+STH*W(I)!由老的GD->新的GD
                150 S(I)=GC(I)+GD(I)
        ...
        SUBROUTINE UPDATE (N,NPT,BMAT,ZMAT,IDZ,NDIM,VLAG,BETA,KNEW,W)!求解算法子程序
                IMPLICIT REAL(8) (A-H,O-Z)
...
PROGRAM test_newuoa!主程序
        use newuoa_module
        implicit real*8 (a-h,o-z)
        integer*4 IP,NF,I
        real*8,allocatable :: X(:),Y(:),XSQ
        real*8 :: RHOBEG
        external CALFUN
        external CALGREY
        character(len=15) :: a(94)=(/'arglina   ',&
                'arglina4  ','arglinb   ','arglinc   ','argtrig   ',&
                'arwhead   ','bdqrtic   ','bdqrticp  ','bdvalue   ',&
                'brownal   ','broydn3d  ','broydn7d  ','brybnd    ',&
                'chainwoo  ','chebquad  ','chnrosnb  ','chpowellb ',&
                'chpowells ','chrosen   ','cosine    ','cragglvy  ',&
                'cube      ','curly10   ','curly20   ','curly30   ',&
                'dixmaane  ','dixmaanf  ','dixmaang  ','dixmaanh  ',&
                'dixmaani  ','dixmaanj  ','dixmaank  ','dixmaanl  ',&
                'dixmaanm  ','dixmaann  ','dixmaano  ','dixmaanp  ',&
                'dqrtic    ','edensch   ','eg2       ','engval1   ',&
                'errinros  ','expsum    ','extrosnb  ','exttet    ',&
                'firose    ','fletcbv2  ','fletcbv3  ','fletchcr  ',&
                'freuroth  ','genbrown  ','genhumps  ','genrose   ',&
                'indef     ','integreq  ','liarwhd   ','lilifun3  ',&
                'lilifun4  ','morebv    ','morebvl   ','ncb20     ',&
                'ncb20b    ','noncvxu2  ','noncvxun  ','nondia    ',&
                'nondquar  ','penalty1  ','penalty2  ','penalty3  ',&
                'penalty3p ','powellsg  ','power     ','rosenbrock',&
                'sbrybnd   ','sbrybndl  ','schmvett  ','scosine   ',&
                'scosinel  ','serose    ','sinquad   ','sparsine  ',&
                'sparsqur  ','spmsrtls  ','srosenbr  ','stmod     ',&
                'tointgss  ','tointtrig ','tquartic  ','trigsabs  ',&
                'trigssqs  ','trirose1  ','trirose2  ','vardim    ',&
                'woods     '/)      
        character(len=15) :: PROBLEM 
        ...
        DO IP=1,94
            ...
            NF=0
            CALL newuoa_min(CALFUN,CALGREY,NF,X, RHOBEG=1.0D0, RHOEND=1.0D-6, IPRINT=3, &
                MAXFUN=10000,PROBLEM=a(IP),FLAG_Epsilonk=2)!针对没一个问题，调用求解算法求解

            PRINT 10, IP
            10 FORMAT (/4X,'PROBLEM:',I6)

            CALL evalfun(PROBLEM,SIZE(X),X,F,INFO)
            ...
            WRITE(1,*)NF,F,X(:)!将结果写入文件

        END DO
        CLOSE(1)
END PROGRAM

subroutine CALFUN(X,F,PROBLEM)!F(X)
        integer*4 :: N,INFO,I
        real*8 :: X(:),F
        character(len=15) :: PROBLEM
        N=size(X)
        F=0.0
        CALL evalfun(PROBLEM,N,X,F,INFO)
...
subroutine CALGREY(X,K,G,FLAG,EPS,EPS1,EPS2,RAND,PROBLEM,FLAG_Epsilonk)!G(X,K)

        implicit real*8 (a-h,o-z)
        real*8 :: X(:),G,PI,PP,EPS(:),EPS1(:),EPS2(:),E,RAND(:),FF
        integer*4 :: K,I,N,FLAG,INFO,FLAG_Epsilonk
        double precision :: Epsilonk
        character(len=15) :: PROBLEM
        
        IF (FLAG_Epsilonk .EQ. 0)THEN!无噪声
            G=0
        END IF

        IF (FLAG_Epsilonk .EQ. 1)THEN!单纯Laplace噪声
            N=SIZE(X)

            IF (K .GT. FLAG)THEN
                FLAG=K
                CALL Laplace(100.0D0/K,Epsilonk)
                EPS(K)=Epsilonk
            ELSE
                Epsilonk=EPS(K)
            END IF
            
            CALL evalfun(PROBLEM,N,X,G,INFO)
            
            G=G+EPS(K)
        END IF

        IF (FLAG_Epsilonk .EQ. 2)THEN!混合噪声
            N=SIZE(X)
  
            IF (K .GT. FLAG)THEN
                CALL Laplace(100.0D0/K,Epsilonk)
                EPS1(K)=Epsilonk*100.0D0

                call random_seed()
                call random_number(Epsilonk)
                Epsilonk=(Epsilonk-0.5D0)*2*(1.0D0/K)
                EPS2(K)=Epsilonk
            ...
            IF (RAND(K) .GT. 0.5)THEN!混合噪声的加法
                G=G+EPS1(K)
            ELSE
                IF (K .GT. 2*N+1)THEN!混合噪声的乘法
                    FF=0.0D0
                    CALL evalfun(PROBLEM,N,X,FF,INFO)
                    Epsilonk=EPS2(K)
                    G=G+EPS2(K)*(G+FF)
                ELSE
                    G=G+EPS1(K)!混合噪声的加法
...
subroutine Laplace(beta,Epsilonk)!Laplace噪声函数
        implicit none
...
!subroutine CALJDGETDF(DELTA,g,H,xx,N)
!      implicit real*8 (a-h,o-z)
!      real*8 :: xx(:),g(:),H(:,:),DELTA,X_K(N),g_K(N),D_K(N),alpha_K,belt_K
!      integer*4 :: I,N,K
!      
!      
!      
...
!      integer(kind=4), external :: mod
     ...
     ceiling(sqrt(dble(n))))
      ...
      ENDDO       
!
      IF (trim(ful) .EQ. 'peng') THEN
           tmp = 0.0D0
           DO i = 1, n
               tmp = tmp+y(i)**2
           ENDDO
           f = tmp
      ELSEIF (trim(ful) .EQ. 'arglina') THEN
...
! See Powell, 2003, On trust region methods for unconstrained
! minimization without derivatives, Problem 2. Also see CUTEr.
! NOTICE: the definitions in the above references are different,
! the difference being -4Y(I)+3 V.S. (-4Y(I)+3)^2. We call the former
! one BDQRTICP (see below). The original definition
! is in Conn, Gould Lescrenoer, Toint, 1994, Performance of a
! Multifrontal scheme for partially seperable optimization, Problem
! 61. But the paper is not available for now.
          f = 0.0D0
          DO i = 1, n-4
...
! Method for Convex Minimization, Problem 6, and Luksan, Vicek,
...
! Difference Algorithms for Solving Sparse Nonlinear Systems of Equations"
...
! regularized Newton Method for Convex Minimization, Problem 6,
! and Luksan, Vicek, Sparse and partially separable test problems for
...
! regularized Newton Method for Convex Minimization, Problem 3.
          f = 0.0D0
          DO i = 2, n
...
! See Ying-Jie Li, Dong-Hui Li, Truncated
! regularized Newton Method for Convex Minimization, Problem 4.
          f = 0.0D0
          DO i = 2, n
...
!            for unconstrained optimization without derivatives", 2004,
!            section 8.
          ENDIF
          DO i = 1, n-2
...
! In claasical Rosenbrock function, alpha = 100.0D0
! When alpha = 4.0D0, this function is essentially the same as Chrosen,
...
! Difference Algorithms for Solving Sparse Nonlinear Systems of Equations"
! by Guangye Li (SIAM Journal on Numerical Analysis, 1988).
! The formulation in the paper seems not correct. The cases with j=3 and
...
! by Guangye Li (SIAM Journal on Numerical Analysis, 1988).
      ...
      endsubroutine evalfun
                        ...
                        CALL CALGREY(X,NF-1,G2,FLAG,EPS,EPS1,EPS2,RAND,PROBLEM,FLAG_Epsilonk)!将新的插值点处的
