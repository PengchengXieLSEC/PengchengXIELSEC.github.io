...
def NEWUOAStep1():
    ...
    gamma = array([0] * m)  # array类型
    Xn = zeros((n, m))  # array类型
    Fn = array([0] * m)  # array类型
    x0 = xb  # array类型
    Xn[:, 0] = x0  # array类型
    Fn[0] = F(Xn[:, 0])
    I = eye(n)  # array类型
    ...
    delta = rho  # 浮点型
    Krho = 0  # 整型
    D3 = array([0] * 3)  # array类型
    QF3 = array([0] * 3)  # array类型
    # ============= global end ==========================
    # #生成插值点
    Delta = array([0] * n)  # array类型
    if m >= 2 * n + 1:
        for i in range(0, n):
            Xn[:, i + 1] = x0 + rho * I[:, i]
            Fn[i + 1] = F(Xn[:, i + 1])
...
def NEWUOAStep9():
    ...
    def eta(u):  # u为array类型
        ...
        print(dot(X.T,u))
        print(u)
        return H[0:m, t - 1] * dot(X.T, u)#5*1   5*1

    def DDlu(u):  # u为array类型
        return dot(X, eta(u).T)#2*5
    ...
    if not isnan(s).any():
        ...
        if abs(alpha * beta + tau2) <= 0.8 * tau2:
            ...
            for j in range(0, 50):
                ...
                wv = w - v
                Hwv = H * wv
                DEN[j] = alpha * (0.5 * N0 ** 4 - dot(xx1, xx0) ** 2 + 0.5 * N1 ** 4) - alpha * wv.T * Hwv + (
                    Hwv[t]) ** 2
